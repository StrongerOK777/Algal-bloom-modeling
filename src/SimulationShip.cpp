...
bool isGreenBloom(const cv::Vec3b& px) {
...
bool isWhiteLand(const cv::Vec3b& px) {
	const int b = static_cast<int>(px[0]);
	const int g = static_cast<int>(px[1]);
	const int r = static_cast<int>(px[2]);
...
cv::Mat bloomImageToLandMask(const cv::Mat& color) {
...
cv::Mat bloomImageToMask(const cv::Mat& color) {
...
cv::Mat maskToBloomMap(const cv::Mat& mask, const cv::Mat& landMask) {
...
cv::Mat estimateFlowFromBloomPair(const cv::Mat& bloom1, const cv::Mat& bloom2) {
	cv::Mat prevGray;
	cv::Mat currGray;
...
void clearInsideSecondaryRing(cv::Mat& mask, const SimulationShip::ShipConfig& cfg) {
...
cv::Mat advectMaskByFlow20Min(const cv::Mat& currentMask, const cv::Mat& flow) {
...
int clearNearestBloomPixelsInSecondary(cv::Mat& mask, const SimulationShip::ShipConfig& cfg, int ships) {
...
void countDangerPixels(
	const cv::Mat& mask,
	const SimulationShip::ShipConfig& cfg,
	int& innerCount,
	int& annulusCount
) {
	innerCount = 0;
...
bool generateShipGif(const std::filesystem::path& frameDir, const std::filesystem::path& gifPath) {
