...
cv::Mat toGrayPreviewU8(const std::vector<float>& band, int width, int height) {
	float lo = 0.0f;
	float hi = 1.0f;
...
std::vector<float> calcNdvi(const TifCube& cube) {
...
cv::Mat buildNdviPseudoColor(const std::vector<float>& ndvi, int width, int height) {
...
cv::Mat buildBloomMap(const std::vector<float>& ndvi, int width, int height) {
...
std::vector<float> buildBloomBinaryFromNdvi(const std::vector<float>& ndvi) {
...
cv::Mat buildBloomMapFromBinary(const std::vector<float>& bloom, int width, int height) {
	const cv::Vec3b water(255, 179, 128);
	const cv::Vec3b land(255, 255, 255);
	const cv::Vec3b algae(0, 128, 0);
	cv::Mat out(height, width, CV_8UC3, water);
...
std::vector<float> moveBloomPixels(
...
void addPanelTitle(cv::Mat& img, const std::string& title) {
	cv::putText(
		img,
		title,
		cv::Point(20, 38),
...
cv::Mat buildForecastGrid(const std::vector<cv::Mat>& frames) {
