...
bool isBloomPixel(const cv::Mat& image, int row, int col) {
...
cv::Ptr<cv::freetype::FreeType2> getChineseTextRenderer() {
...
void drawWarningLinesBottomRight(cv::Mat& image, const std::vector<std::string>& lines) {
...
bool generateEarlyWarningGif(const std::filesystem::path& outputDir) {
	...
	const std::string warningGifCmd =
		...
		"\" -loop 0 \"" + warningGifPath.string() + "\" >/dev/null 2>&1";
