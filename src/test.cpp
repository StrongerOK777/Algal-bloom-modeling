...
std::pair<std::string, std::string> buildLabelText(const std::string& srcPath) {
...
bool drawOverlayLabel(const char* outputPath, const std::string& srcPath) {
...
std::vector<float> truncatedLinearStretch(
	const float* gray,
	size_t pixelCount,
...
int runTestFigure(const std::string& srcPath, const char* outputPath, int stretchMode) {
	GDALAllRegister();
